...
fls_idrisi <- list.files(paste0(filepath_vector,"/Idrisi_rst"), full.names = TRUE, pattern ="rst")
...
all_tr_areas <- readOGR(dsn = paste0(filepath_base,"/",fls_shp[1],".shp"), layer = fls_shp[1])

nrw <- c(nrow(all_tr_areas@data))

for (i in seq(2,length(fls_shp))){
  print(i)
  act_shape <- readOGR(dsn = paste0(filepath_base,"/",fls_shp[i],".shp"), layer = fls_shp[i])
...
# add training areas for clouds and shadow
